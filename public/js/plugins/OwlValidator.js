const _OwlValidator = {
        ...
        for (let w = 0; w < wrappers.length; w++) {
            ...
            if (eventType instanceof Array) {
                for(e = 0; e < eventType.length; e++) {
                    wrappers[w].addEventListener(eventType[e], event => { _OwlValidator.prepare(options, event.currentTarget) });
                }
            } else {
                wrappers[w].addEventListener(eventType, event => { _OwlValidator.prepare(options, event.currentTarget) });
            }
        }
    },
    prepare: (options, wrapper) => {
        ...
        _OwlValidator.specify(options, rules, field, wrapper);
    },
    specify: (options, rules, field, wrapper) => {
        ...
        _OwlValidator.respond(options, wrapper, details);
    },
    validate: {
        required: (rule, field) => {
...
class OwlValidator {
    constructor (options) {
        ...
        if (arguments[0] && typeof arguments[0] === "object") {
            this.options = _OwlValidator.extend(this.options, arguments[0]);
        }
    }
    initialize() {
        _OwlValidator.set(this.options);
    }
}
